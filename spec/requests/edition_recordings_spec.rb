...
RSpec.describe "Edition recordings" do
  def edition_holding_audio(bytes: "the-exact-bytes")
    edition = create(:edition)
    create(:edition_recording, edition: edition).store(bytes, voice: "a-voice")

    edition
  end

  # Eight of the examples below differ only in the Range they send, so the
  # signing in, the fixture and the request are a helper taking the one thing
  # that varies — which is what .claude/rules/testing.md asks for instead of a
  # before block.
  def seek(range, bytes: "0123456789")
    sign_in
    get edition_recording_path(edition_holding_audio(bytes: bytes)),
      headers: { "Range" => range }
  end

  def record_more_editions_than_allowed
    sign_in
    Array.new(6) { create(:edition) }
      .each { |edition| post edition_recording_path(edition) }
  end
  ...
  describe "asking for a recording" do
    ...
    it "turns away a reader recording one edition after another" do
      record_more_editions_than_allowed
