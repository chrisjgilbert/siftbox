...
class Blog::Fetch
  ...
  #
  # Download's own marker under a name from this side of the fence, rather
  # than a second one beside it. Blog::Poll goes on saying Blog::Fetch::
  # UNCHANGED and goes on not knowing Download exists; what goes is the pair
  ...
  def result
    body = download.body
    return UNCHANGED if body.equal?(UNCHANGED)
    return if body.nil?
  ...
  # Through Utf8 for the reason it gives: a document tagged as something it is
  # not fails the parser outright, and recoding only the bad runs keeps the
  # accents in a feed that is UTF-8 apart from one stray byte.
  def fetched(body)
    Fetched.new(
      document: Utf8.new(body.bytes).text, etag: body.etag,
